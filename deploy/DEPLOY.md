...
```bash
git clone <repo-url> /opt/hello-xiao-a
cd /opt/hello-xiao-a
git checkout main
```
...
仓库已提供 `scripts/backup.sh`，它会备份 PostgreSQL 和 etcd，并清理超过保留天数的文件。请从仓库根目录执行：
...
默认备份目录为 `backups/`，默认保留 7 天。可通过环境变量覆盖：

```bash
BACKUP_DIR=/mnt/agent-backups RETAIN_DAYS=14 ./scripts/backup.sh
```
### 5.2 配置定时任务
...
# 每日凌晨 2 点备份
0 2 * * * /opt/hello-xiao-a/scripts/backup.sh >> /var/log/agent_backup.log 2>&1
```
...
docker compose --env-file .env.prod -f docker-compose.prod.yml stop milvus-standalone
docker compose --env-file .env.prod -f docker-compose.prod.yml cp backups/etcd_20260726_020000.db etcd:/etcd/restore.db
docker compose --env-file .env.prod -f docker-compose.prod.yml exec etcd etcdctl snapshot restore /etcd/restore.db --data-dir /etcd
docker compose --env-file .env.prod -f docker-compose.prod.yml start milvus-standalone
```
...
# 特定时间段
docker compose --env-file .env.prod -f docker-compose.prod.yml logs --since 1h api

# Nginx 访问日志
docker compose --env-file .env.prod -f docker-compose.prod.yml exec nginx cat /var/log/nginx/access.log
```
...
- [ ] 定期更新基础镜像（`docker compose --env-file .env.prod -f docker-compose.prod.yml pull && docker compose --env-file .env.prod -f docker-compose.prod.yml up -d`）
...
```bash
# 查看启动日志
docker compose --env-file .env.prod -f docker-compose.prod.yml logs api | tail -50
...
```bash
# 检查 RediSearch 模块
docker compose --env-file .env.prod -f docker-compose.prod.yml exec redis redis-cli -a $REDIS_PASSWORD MODULE LIST
...
docker compose --env-file .env.prod -f docker-compose.prod.yml logs --tail 0 -f api  # 看实时
# 然后 truncate 旧日志
